...
class Wots {
  ...
  static const int messageDigitCount = 64; // 256 bits / log2(16)
  static const int checksumDigitCount = 3; // ceil(log_16(960)) = 3
  static const int totalDigitCount = messageDigitCount + checksumDigitCount; // 67
  static const int n = 32; // bytes per element

  static List<int> _digestIteration(List<int> x) => sha256.convert(x).bytes;
  ...
  static List<int> _digitsWithChecksum(Uint8List msgHash) {
    final d = List<int>.filled(totalDigitCount, 0);
    final nibbles = _toNibbles(msgHash);
    for (int i = 0; i < messageDigitCount; i++) {
      d[i] = nibbles[i];
    }
    int csum = 0;
    for (int i = 0; i < messageDigitCount; i++) {
    ...
    d[messageDigitCount] = (csum >> 8) & 0x0f;
    d[messageDigitCount + 1] = (csum >> 4) & 0x0f;
    d[messageDigitCount + 2] = csum & 0x0f;
    return d;
  }
  ...
  static (List<Uint8List> sk, List<Uint8List> pk) keygen(Uint8List seed) {
    ...
    for (int i = 0; i < totalDigitCount; i++) {
      ...
      for (int j = 0; j < w - 1; j++) {
        v = Uint8List.fromList(_digestIteration(v));
      }
